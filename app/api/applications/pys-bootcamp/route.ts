...
import { trainingCampApplicationSchema } from "@/lib/application-schema";
...
export async function POST(request: Request) {
  ...
  const submission = await parseApplicationSubmission(request, { withReceipt: true });
  if (!submission.ok) {
    return NextResponse.json({ message: submission.message, issues: submission.issues }, { status: 400 });
  }

  const parsed = trainingCampApplicationSchema.safeParse(submission.input);
  if (!parsed.success) {
  ...
  try {
    const result = await saveTrainingCampApplication(parsed.data, submission.photo, submission.receipt!);
    ...
    if (error instanceof Error && error.message === "DUPLICATE_APPLICATION") {
      return NextResponse.json({ message: "An application using this email, WhatsApp number or CNIC has already been received." }, { status: 409 });
    }
    console.error("PYS Bootcamp application error", error);
